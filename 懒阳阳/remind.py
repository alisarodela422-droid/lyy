...
def parse_date(value) -> datetime | None:
    ...
    if value is None:
        return None
    # 毫秒时间戳（日期字段类型返回整数）。
    # 飞书日期字段存的是「北京时间零点」的时间戳，所以必须用北京时间解析。
    ...
    if isinstance(value, str):
        ...
        if not value:
            return None
        for fmt in ("%Y-%m-%d", "%Y/%m/%d", "%Y.%m.%d", "%Y年%m月%d日", "%Y-%m-%d %H:%M:%S", "%Y/%m/%d %H:%M"):
            try:
...
def main() -> int:
    ...
    # 目标日期（用北京时间）
    target = datetime.now(BEIJING_TZ) + timedelta(days=days_ahead)
    target_date = target.date()
    print(f"[时间] 北京时间今天 = {datetime.now(BEIJING_TZ).strftime('%Y-%m-%d %H:%M')}，目标日期 = {target_date}")
    print(f"[数据] 合作表共 {len(records)} 条记录")
    ...
    for r in records:
        ...
        shoot_raw = f.get(shoot_field) if shoot_field else None
        publish_raw = f.get(publish_field) if publish_field else None
        print(f"  - {brand}: 拍摄日期={shoot_raw!r} 发布日期={publish_raw!r}")

        if shoot_field:
            d = parse_date(shoot_raw)
            if d and d.date() == target_date:
                to_shoot.append(brand)

        if publish_field:
            d = parse_date(publish_raw)
            if d and d.date() == target_date:
